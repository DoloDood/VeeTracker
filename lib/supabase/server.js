...
export async function getCurrentUser() {
  try {
    ...
    if (authError) {
      console.error('[getCurrentUser] supabase auth.getUser() error:', authError.message);
    }
    if (!user) return null;

    console.log('[getCurrentUser] got supabase user:', user.id, user.email);

    const res = await sql`SELECT username FROM profiles WHERE id = ${user.id}`;
    console.log('[getCurrentUser] profile lookup succeeded, rows:', res.rows.length);
    return { id: user.id, email: user.email, username: res.rows[0]?.username || null };
  } catch (err) {
    console.error('[getCurrentUser] THREW:', err.message, err.stack);
    throw err;
  }
}
